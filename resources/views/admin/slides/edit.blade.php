...
<style>
  :root{
    --ink:#0b1220; --muted:rgba(15,23,42,.62);
    --line:rgba(15,23,42,.12); --line2:rgba(15,23,42,.18);
  ...
  .title{ font-size:1.55rem; font-weight:950; letter-spacing:-.02em; color:var(--ink); }
  .subtitle{ margin-top:8px; color:var(--muted); font-weight:700; font-size:.95rem; }
  .panel{
    margin-top: 14px;
  ...
  .panel-head .h{ font-weight:950; color:var(--ink); }
  .body{ padding: 16px; }
  ...
  .muted{ color: rgba(15,23,42,.60); font-weight: 800; font-size:.92rem; }
</style>

<div class="wrap">
  <div>
    <div class="title">Editar slide</div>
    <div class="subtitle">Actualiza imagen, link, orden y estado.</div>
  </div>

  <div class="panel">
    <div class="panel-head">
      <div class="h">Slide #{{ $slide?->id ?? '—' }}</div>
      <a class="btn btn-ghost" href="{{ route('admin.slides.index') }}">← Volver</a>
    </div>

    <form class="body" method="POST" action="{{ route('admin.slides.update', $slide->id) }}" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <div class="grid">
        <div style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div>
            ...
            <input class="input" type="text" name="link" value="{{ old('link', $slide->link ?? $slide->url ?? '') }}">
          </div>

          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
            <div>
              <label class="field-label">Orden</label>
              <input class="input" type="number" name="sort_order" value="{{ old('sort_order', $slide->sort_order ?? $slide->sort ?? 0) }}" min="0">
            </div>

            <div style="display:flex; align-items:flex-end; gap:10px; padding-bottom:6px;">
              <input id="is_active" name="is_active" type="checkbox" class="rounded" value="1"
                     {{ old('is_active', (int)($slide->is_active ?? 0)) ? 'checked' : '' }}>
              <label for="is_active" class="field-label" style="margin:0;">Activo</label>
            </div>
          </div>

          <div>
            <label class="field-label">Imagen (reemplazar)</label>
            <input id="image" class="input" style="padding:.75rem 1rem;" type="file" name="image" accept="image/*">
            <div class="muted" style="margin-top:8px;">Si no seleccionas archivo, se conserva la imagen actual.</div>
          </div>
        </div>

        <div>
          <div class="preview">
            <div class="ph" id="ph" style="{{ $imgUrl ? 'display:none;' : '' }}">
              <div class="muted">Sin imagen</div>
            </div>
            <img id="imgPreview" src="{{ $imgUrl ?? '' }}" alt="" style="{{ $imgUrl ? '' : 'display:none;' }}">
            <div class="meta">
              <div style="font-weight:950; color:rgba(15,23,42,.88);" id="pvTitle">—</div>
              <div class="muted" id="pvLink" style="margin-top:6px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">—</div>
            </div>
          </div>
        </div>
      </div>
