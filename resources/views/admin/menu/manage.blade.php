  ...
  <div class="mm-wrap">
    <div class="mm-shell">
      ...
      <div class="mm-head">
        <div class="mm-head-top">
          <div class="min-w-0">
            ...
            <p class="mm-sub">
              Aquí puedes crear sub-opciones y asignarles PDF. Los cambios se reflejan automáticamente en el menú
              superior.
            </p>
          </div>
      ...
      <div class="mm-body">
        ...
        <div class="mm-card">
          ...
          <div class="mm-card-body" style="display:grid; gap:18px;">
            <div class="mm-section">
              <div class="mm-section-title">Subir / reemplazar archivo del nodo actual</div>
              <div class="mm-section-sub" style="font-size:12px; color:#64748b; margin-bottom:8px;">
                Acepta PDF, Word (.doc/.docx), Excel (.xls/.xlsx) e imágenes (.jpg/.png/.gif/.webp/.svg).
              </div>

              <form method="POST" action="{{ route('admin.menu.upload', $node) }}" enctype="multipart/form-data"
                class="mm-upload-row">
                @csrf
              ...
              @if($nodeCurrentUrl)
                <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:12px;">
                  ...
                  <form method="POST" action="{{ route('admin.menu.pdf.destroy', $node) }}"
            ...
            <div class="mm-section">
              ...
              <form method="POST" action="{{ route('admin.menu.update', $node) }}" class="mm-edit-grid">
                ...
                <input name="label" value="{{ $node->label }}" class="mm-input" placeholder="Label" required />

                <input name="sort" value="{{ (int) $node->sort }}" class="mm-input" placeholder="Sort" />

                <input name="url" value="{{ $node->url }}" class="mm-input" placeholder="url (opcional)" />
        ...
        <div class="mm-card">
          ...
          <div class="mm-card-body">
            <form method="POST" action="{{ route('admin.menu.children.store', $node) }}" class="mm-create">
              ...
              <input type="text" name="label" value="{{ old('label') }}" placeholder="Ej. Inventarios" class="mm-input"
                required>
                ...
                <div class="mm-actions">
                  ...
                  <div class="mm-section">
                    ...
                    <div class="mm-tip" style="margin-top:12px;">
                      Entra aquí para crear opciones hijas de <code>{{ $child->label }}</code> y aplicarles la misma lógica
                      de PDF o navegación.
                    </div>
                  </div>
                  ...
                  {{-- ✅ Subir archivo --}}
                  <div class="mm-section">
                    <div class="mm-section-title">Subir / reemplazar archivo</div>
                    <div class="mm-section-sub" style="font-size:12px; color:#64748b; margin-bottom:8px;">
                      Acepta PDF, Word, Excel e imágenes.
                    </div>

                    <form method="POST" action="{{ route('admin.menu.upload', $child) }}" enctype="multipart/form-data"
                      class="mm-upload-row">
                      @csrf
                      <input type="file" name="pdf"
                        accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif,.webp,.svg,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,image/*"
                    ...
                    @if($currentUrl)
                      <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:12px;">
                        ...
                        <form method="POST" action="{{ route('admin.menu.pdf.destroy', $child) }}"
                          onsubmit="return confirm('¿Quitar el PDF de esta opción?');">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="mm-btn mm-btn-danger">
                  ...
                  <div class="mm-section">
                    ...
                    <form method="POST" action="{{ route('admin.menu.update', $child) }}" class="mm-edit-grid">
                      ...
                      <input name="label" value="{{ $child->label }}" class="mm-input" placeholder="Label" required />

                      <input name="sort" value="{{ (int) $child->sort }}" class="mm-input" placeholder="Sort" />

                      <input name="url" value="{{ $child->url }}" class="mm-input" placeholder="url (opcional)" />
        ...
        <div class="mm-tip">
          Tip: esta vista vive en <code>/admin/menu/{id}/manage</code> y está pensada para usarse desde el engrane del
          menú.
        </div>
