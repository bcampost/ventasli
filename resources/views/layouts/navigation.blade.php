...
<nav class="v-nav">
  <div class="v-nav-wrap">
    ...
    <button type="button" class="v-burger" id="vBurger" aria-label="Abrir menú" aria-expanded="false"
    ...
    <div class="v-mobile-panel" id="vMobilePanel">
              ...
              <div class="v-dd">
                @if(!$isProducts)
                  @foreach($root->children as $child)
                    ...
                    <a href="{{ $childHref }}" @if($isPdfUrl($childHref))
                    onclick="event.preventDefault(); window.openPdfPreview(@js($childHref), @js($child->label));" @endif>
                ...
                @else
                  @foreach($root->children as $child)
                    ...
                    <div class="v-dd-item {{ $childHasKids ? 'has-kids' : '' }}">
                      @if($childHasKids)
                        <button type="button" class="v-dd-toggle" onclick="return handleSubmenuToggle(event, this)">
                          <span>{{ $child->label }}</span>
                      ...
                      @if($childHasKids)
                        <div class="v-dd v-dd-sub">
                          @foreach($child->children as $g)
                            ...
                            @endphp
                            <a href="{{ $gHref }}" @if($isPdfUrl($gHref))
                            onclick="event.preventDefault(); window.openPdfPreview(@js($gHref), @js($g->label));" @endif>
                              <span>{{ $g->label }}</span>
      ...
      <div class="v-mobile-links">
        <div class="v-mobile-links-group" id="vMobileLinksGroup">
          <button type="button" class="v-mobile-links-toggle" onclick="toggleMobileLinksGroup()" aria-expanded="false"
          ...
          <div class="v-mobile-links-list" id="vMobileLinksList">
            @foreach($quickLinks as $l)
              ...
              <a class="v-mobile-link" href="{{ $finalHref }}" @if($ext) target="_blank" rel="noopener" @endif>
    ...
    <div class="v-right">
      ...
      <div class="v-search">
        <form method="GET" action="{{ route('search.global') }}">
          <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar en todo..." autocomplete="off">
        </form>
      </div>
    </div>
